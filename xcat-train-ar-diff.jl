...
# randseed = 2
shortcode = "3layer-ar-xcat-3iter-1800-epochs"
...
for i = 1:nouter
    ...
    # file = joinpath(pwd(), "ckpt/last-3layer-ar-xcat-3iter-1500-epochs"*string(i)*".bson")
    # @load file cnn_cpu
    # cnn = cnn_cpu |> gpu
    push!(cnn_list, cnn)
end
...
function loss(A, xtrue, ynoisy, r, Asum, xrecon, cnn_list, β; niter = 1, nouter = nouter)
    xout = regem(A, ynoisy, r, Asum, xrecon, cnn_list[1], β; niter)
    for i = 1:nouter-1
        xout = regem(A, ynoisy, r, Asum, xout, cnn_list[i+1], β; niter)
    end
    return nrmse(xout, xtrue) # use high frequency error norm
end
...
for e = 1:nepoch
    ...
    for idx = 1:train_loader.num
        ...
        spect, xtrue, yi, ri, mumap, psf = grab_data(train_loader, idx)
        train_loss[e, idx] = loss(A_train_list[idx], xtrue, yi, ri,
        ...
        for i = 1:nouter
            ...
            gs = gradient(ps) do
                loss(A_train_list[idx], xtrue, yi, ri, Asum_train_list[idx],
    ...
    for idx = 1:valid_loader.num
        ...
        spect, xtrue, yi, ri, mumap, psf = grab_data(valid_loader, idx)
        valid_loss[e, idx] = loss(A_valid_list[idx], xtrue, yi, ri,
                                  Asum_valid_list[idx], spect, cnn_list, β; niter = 1)
